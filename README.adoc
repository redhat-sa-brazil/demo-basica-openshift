...
node('maven') {
    ...
    deploy(app + '-dev', app + '-qa', app, 'homologacao', token)

    stage 'Wait for approval'
    input 'Aprove to production?'

    stage 'Deploy to Prod'
    deploy(app + '-dev', app + '-prd', app, 'producao', token)
}
...
def deploy(String origProject, String project, String app, String ambiente, String ocpUrl, String token){
    ...
    sh "echo 'usuario=teste-${ambiente}\nurl=http://${ambiente}-url' > hello${ambiente}.properties"
    sh "oc create configmap hello${ambiente}conf --from-file=hello${ambiente}.properties"
...
def appDeploy(String app, String project, String ambiente){
    ...
    sh "oc volumes dc/${app} --add --name=hello${ambiente}conf --mount-path=/opt/ --configmap-name=hello${ambiente}conf --type=configmap"
    sh "oc expose service ${app} -n ${project} || echo 'Service already exposed'"
}
----
