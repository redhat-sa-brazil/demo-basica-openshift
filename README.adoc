...
node('maven') {
	...
	def gitUrl = 'https://github.com/guaxinim/demo-openshift.git'
        def ocpUrl = 'https://master.rhpds311.openshift.opentlc.com'
	def token = 'byG7Rb1UfQbuzpaInOT_zwoEzCk4r4SrapFBG7XpcLc'
...
def buildApp(String project, String gitUrl, String app, String ocpUrl, String token){
    ...
    sh "echo 'usuario=ambiente-dev' > hello.properties"
    sh "oc create configmap hellodevconf --from-file=hello.properties"
    sh "oc set volume dc/${app} --add --name=hellodevconf --mount-path=/data --configmap-name=hellodevconf --type=configmap"
    sh "oc expose service ${app} -n ${project} || echo 'Service ja existe'"
    sh "sleep 10"
}

def projectSet(String project, String ocpUrl, String token) {
    sh "oc login --insecure-skip-tls-verify=true ${ocpUrl} --token=${token}"
    sh "oc new-project ${project} || echo 'Projeto ja existe'"
...
def deploy(String origProject, String project, String app, String ambiente, String ocpUrl, String token){
    ...
    sh "echo 'usuario=ambiente-${ambiente}' > hello.properties"
...
def appDeploy(String app, String project, String ambiente){
    ...
    sh "oc set volume dc/${app} --add --name=hello${ambiente}conf --mount-path=/data --configmap-name=hello${ambiente}conf --type=configmap"
    sh "oc expose service ${app} -n ${project} || echo 'Service ja existe'"
}
----
